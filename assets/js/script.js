  ...
  shopData.forEach((channel) => {
    ...
    <div class="main-output-channel">
        ...
        <div class="main-output-channel-config">
            <p class="main-output-channel-config-provision">${channel.percentageProvision.toFixed(
              2
            )}% + ${channel.flatProvision.toFixed(2)}€</p>
  ...
  for (let i = 0; i < priceValueElements.length; i++) {
    if (firstInputElement.value == 0 && secondInputElement.value == 0) {
      ...
      if (shopData[i].shopName === "Cardmarket") {
        ...
        priceValueElements[i].value = (
          (Number(firstInputElement.value) + Number(secondInputElement.value) + shopData[i].flatProvision) /
            percentageValue -
          (shopData[i].shippingCost > SHIPPING_COST
            ? (shopData[i].shippingCost - SHIPPING_COST) / (1 - shopData[i].percentageProvision / 100)
            : 0)
        ).toFixed(2);
      } else {
        const percentageValue = 1 - VAT.value / (1 + VAT.value) - shopData[i].percentageProvision / 100;
        ...
        priceValueElements[i].value = (
          (Number(firstInputElement.value) +
            Number(secondInputElement.value) +
            shopData[i].shippingCost * (shopData[i].percentageProvision / 100) +
            shopData[i].flatProvision) /
            percentageValue +
          (shopData[i].shippingCost < SHIPPING_COST
            ? (SHIPPING_COST - shopData[i].shippingCost) /
              (1 - (shopData[i].percentageProvision / 100) * (1 + VAT.value))
            : 0)
        ).toFixed(2);
      }
    } else {
      if (shopData[i].shopName === "Cardmarket") {
        ...
        priceValueElements[i].value = (
          (Number(firstInputElement.value / (1 + VAT.value)) +
            Number(secondInputElement.value) +
            shopData[i].flatProvision) /
            percentageValue -
          (shopData[i].shippingCost > SHIPPING_COST
    ...
    if (firstInputElement.value == 0 && secondInputElement.value == 0) {
      ...
      if (shopData[i].shopName === "Cardmarket") {
        ...
        priceValueElements[i].value = (
          ...
          shopData[i].flatProvision -
          Number(secondInputElement.value) +
          (shopData[i].shippingCost > SHIPPING_COST ? (shopData[i].shippingCost - SHIPPING_COST) / (1 + VAT.value) : 0)
        ).toFixed(2);
      } else {
        const channelProvision =
          (Number(firstInputElement.value) + shopData[i].shippingCost) * (shopData[i].percentageProvision / 100);
        ...
        priceValueElements[i].value = (
          ...
          shopData[i].flatProvision -
          Number(secondInputElement.value) -
          (shopData[i].shippingCost < SHIPPING_COST ? (SHIPPING_COST - shopData[i].shippingCost) / (1 + VAT.value) : 0)
        ).toFixed(2);
      }
    } else {
      if (shopData[i].shopName === "Cardmarket") {
        ...
        priceValueElements[i].value = (
          ...
          channelProvision -
          shopData[i].flatProvision -
          Number(secondInputElement.value / (1 + VAT.value)) +
          (shopData[i].shippingCost > SHIPPING_COST ? (shopData[i].shippingCost - SHIPPING_COST) / (1 + VAT.value) : 0)
        ).toFixed(2);
      } else {
        const channelProvision =
